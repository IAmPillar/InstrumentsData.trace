...
@property (nonatomic, retain) UIImage *currentColorImgView;
- (void) fixLocations;
@end
...
@synthesize currentColorImgView = _currentColorImgView;

float scale = 1;//fit iphone and ipad(height:460(4s);548(5,5s,6,6plus);1004(ipad))
//@synthesize screenBounds = _screenBounds;
...
- (void) setup {
	...
	self.backgroundColor = [UIColor colorWithRed:0.950 green:0.950 blue:0.950 alpha:1.000];
    
    scale = self.frame.size.height / 460;

    // current color indicator hier.
    KZColorCompareView *colorView = [[KZColorCompareView alloc] initWithFrame:CGRectMake(10, 70, 44, 44)];
	...
	// HS wheel.
	KZColorPickerHSWheel *wheel = [[KZColorPickerHSWheel alloc] initAtOrigin:CGPointMake((self.frame.size.width - 240) / 2 , 110 * scale)];//hanlili
	...
	// brightness slider
	KZColorPickerBrightnessSlider *slider = [[KZColorPickerBrightnessSlider alloc] initWithFrame:CGRectMake(24,277,272,40)];
    ...
    // alpha slider
    KZColorPickerAlphaSlider *alpha = [[KZColorPickerAlphaSlider alloc] initWithFrame:CGRectMake(24, 321, 0, 0)];
...
- (void) fixLocations
{
    ...
    if(self.bounds.size.width < self.bounds.size.height)
    {
        ...
        int sx = 20;
        int sy = 430 * scale - (scale - 1) * 100;//hanlili
        ...
        self.brightnessSlider.frame = CGRectMake(24,377  * scale - (scale - 1) * 150,self.bounds.size.width - 20 - 24,38);//hanlili
        self.alphaSlider.frame = CGRectMake(24, 321, 0, 0);
    }
    else
    {
        ...
        self.brightnessSlider.frame = CGRectMake(300, self.bounds.size.height * 0.5 - 38 - 50, 165, 38);
        self.alphaSlider.frame = CGRectMake(300, self.bounds.size.height * 0.5 + 5 - 50, 0, 0);
    }
    /*
...
- (void) colorWheelColorChanged:(KZColorPickerHSWheel *)wheel
{
	...
	self.selectedColor = [UIColor colorWithHue:hsv.h
									saturation:hsv.s
									brightness:hsv.v//self.brightnessSlider.value
...
//    [_sharedUserDefault setObject: [NSString stringWithFormat:@"%f", rgb.r * 255] forKey:@"penColorRed"];
//    [_sharedUserDefault setObject: [NSString stringWithFormat:@"%f", rgb.g * 255] forKey:@"penColorGreen"];
//    [_sharedUserDefault setObject: [NSString stringWithFormat:@"%f", rgb.b * 255] forKey:@"penColorBlue"];
//    [_sharedUserDefault setObject: [NSString stringWithFormat:@"%f", self.alphaSlider.value * 255] forKey:@"penColorAlpha"];
//    [_sharedUserDefault synchronize];
...
- (void) swatchAction:(KZColorPickerSwatchView *)sender
{
    ...
    //RGBType rgb = rgbWithUIColor(sender.color);
