...
//
//        GZGuideViewController *guide = [[GZGuideViewController alloc] init];
...
//
//    //英文
//    NSString *userDict_english = [plist getUserDictionaryPath:@"ltk-user.dic"];
//    if (![manager fileExistsAtPath:userDict_english]) {
//        NSLog(@"开启线程2——英文用户词典");
//        dispatch_queue_t global_queue = dispatch_get_global_queue(0, 0);
//        dispatch_async(global_queue, ^{
//            [plist saveUserDictionary:@"ltk-user.dic"];
//            dispatch_sync(dispatch_get_main_queue(), ^{
//                NSLog(@"线程2——英文用户词典成功");
//                return;
//            });
...
//            [nav popToViewController:home animated:YES];
    ...
    if ([url rangeOfString:@"UserPhrases"].location != NSNotFound) {
        UIViewController *Rootvc = self.window.rootViewController;
        if ([Rootvc isKindOfClass:[UINavigationController class]]) {
            UINavigationController *nav = (UINavigationController *)Rootvc;
            NSMutableArray *pageArray = [nav.viewControllers mutableCopy];
            ...
            [pageArray insertObject:userPhrases atIndex:1];
            [nav setViewControllers:pageArray animated:NO];

            [nav popToViewController:userPhrases animated:YES];
        }
    }
}
