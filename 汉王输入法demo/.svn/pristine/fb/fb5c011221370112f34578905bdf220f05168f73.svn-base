...
{   
    const CGFloat cornerRadius = 0.0f;
    const CGFloat borderWidth = 0.0f;
...
//    CGPathRef leftFillPath = [[self class] newLeftRoundRectPathForBoundingRect:CGRectMake(0, 0, self.bounds.size.width * 0.5 + 1.0f, self.bounds.size.height) cornerRadius:cornerRadius + 1.0f];
    CGPathRef leftFillPath = [[self class] newLeftRoundRectPathForBoundingRect:CGRectMake(0, 0, 0, 0) cornerRadius:cornerRadius + 1.0f];
    CGContextAddPath(context, leftFillPath);
    [self.oldColor setFill];    
    CGContextFillPath(context);
    CGPathRelease(leftFillPath);
    
    CGPathRef rightFillPath = [[self class] newRightRoundRectPathForBoundingRect:CGRectMake(0, 0, self.bounds.size.width, self.bounds.size.height) cornerRadius:cornerRadius + 1.0f];
...
//    CGPathRef path = [[self class] newLeftRoundRectPathForBoundingRect:CGRectMake(0, 0, self.bounds.size.width * 0.5 + 1.0f, self.bounds.size.height) cornerRadius:7.0];
    CGPathRef path = [[self class] newLeftRoundRectPathForBoundingRect:CGRectMake(0, 0, 0, 0) cornerRadius:7.0];
